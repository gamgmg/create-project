...
// node_modules/.pnpm/registry.npmmirror.com+minimist@1.2.7/node_modules/minimist/index.js
var require_minimist = __commonJS({
  "node_modules/.pnpm/registry.npmmirror.com+minimist@1.2.7/node_modules/minimist/index.js"(exports, module2) {
...
// node_modules/.pnpm/registry.npmmirror.com+kolorist@1.6.0/node_modules/kolorist/dist/esm/index.mjs
var enabled = true;
...
var mergeArrayWithDedupe = (a, b) => [.../* @__PURE__ */ new Set([...a, ...b])];
var deepMerge = (target, newTarget) => {
...
var renderTemplate = (src, dest) => {
  ...
  if (stats.isDirectory()) {
    ...
    for (const file of fs2.readdirSync(src)) {
      renderTemplate(path2.resolve(src, file), path2.resolve(dest, file));
    }
    return;
  }
  ...
  if (filename === "_gitignore" && fs2.existsSync(dest)) {
    const existing = fs2.readFileSync(src, "utf8");
    const newGitignore = fs2.readFileSync(dest, "utf8");
...
async function init() {
  ...
  const root = import_path.default.join(cwd, targetDir);
...
Scaffolding project in ${root}...`);
  ...
  import_fs.default.writeFileSync(import_path.default.resolve(root, "package.json"), JSON.stringify(pkg, null, 2));
  const templateRoot = import_path.default.resolve(__dirname, "template");
  const render = (templateName2) => {
    const templateDir = import_path.default.resolve(templateRoot, templateName2);
...
`);
  if (root !== cwd) {
    console.log(`  ${bold(green(`cd ${import_path.default.relative(cwd, root)}`))}`);
  }
